...
#'
#' @param uids A character vector with protein IDs, or a _data.table_ object
#'             of class *LBCE_dt*, generated by [get_LBCE()].
...
get_proteins <- function(uids, save_folder = NULL){
  ...
  if(is.character(uids)){
    assertthat::assert_that(length(uids) >= 1)
  } else {
    assertthat::assert_that("LBCE_dt" %in% class(uids))
    uids <- uids$protein_id
  }
